...
function createList() {
  if (todoInput.value) {
    ...
    function saveTasksToLocalStorage() {
      const taskElements = document.querySelectorAll(".todo-list");
      const tasks = [];
    
      taskElements.forEach((taskElement) => {
        const taskText = taskElement.querySelector(".todo-list-button").textContent;
        tasks.push(taskText);
      });
    ...
    function loadTasksFromLocalStorage() {
      ...
      tasks.forEach((taskText) => {
        ...
        const listButton = document.createElement("button");
        listButton.classList.add("todo-list-button", "active-btn", "btn");
        listButton.textContent = taskText;
